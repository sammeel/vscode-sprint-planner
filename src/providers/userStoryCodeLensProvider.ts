...
export class UserStoryCodeLensProvider implements vsc.CodeLensProvider {
    constructor(private textProcessor: ITextProcessor) {}
    ...
    provideCodeLenses(
        ...
        const userStoryLines = this.textProcessor.getUserStoryLineIndices(
            lines
        );
        ...
        for (const line of userStoryLines) {
            const us = this.textProcessor.getUserStory(lines, line);
            const range = new vsc.Range(line, 0, line, lines[line].length);

            results.push(
                new vsc.CodeLens(range, {
                    title: `⏏️ Publish to Azure DevOps, ${this.buildExtraInfo(
                        us?.tasks
                    )}`,
