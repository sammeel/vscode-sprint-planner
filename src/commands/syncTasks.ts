        ...
        await vsc.window.withProgress(
            ...
            async (progress) => {
                try {
                    ...
                    if (!us) {
                        return console.log(
                            "Cannot find user story info in that line"
                        );
                    }
                    ...
                    progress.report({
                        increment: 20,
                        message: "Getting User story info...",
                    });

                    const workItemInfos = await this.client.GetWorkItemInfos([
                        us.id!,
                    ]);

                    const workItemInfo = workItemInfos.value[0];
                    ...
                    const vsoTaskIds = taskUrls
                        .map(this.extractTaskId)
                        .filter((x) => x) as number[];
                    const vsoTasks = (
                        await this.client.getTasksInfo(vsoTaskIds)
                    ).map(TaskMapper.fromTaskInfo);

                    progress.report({ increment: 40 });

                    const newTasks = us.tasks
                        .filter((t) => !vsoTaskIds.includes(t.id!))
                        .map((t) => {
                            t.activity =
                                t.activity || this.config.defaultActivity || "";
                            return t;
                        });

                    const mergedTasks = sortBy(
                        [...vsoTasks, ...newTasks],
                        ["activity", "stackRank"]
                    );
                    ...
                    if (err) {
                        if (err.response.status === 404) {
                            vsc.window.showErrorMessage(
                                "User story not found or you don't have permission to read it"
                            );
                        } else {
                            vsc.window.showErrorMessage(
                        ...
                        this.logger.log(
                            inspect(err.response.data, { depth: 3 })
                        );
                        return Promise.resolve();
                    }
                } finally {
